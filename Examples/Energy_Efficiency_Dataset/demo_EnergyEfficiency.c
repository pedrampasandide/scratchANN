...
int main()
{
    ...
    /* INPUT:
    for example, = 0.8 means 80% of data for train and 20% for validation (actually for test), I just called it here validation*/
    ...
    0.25/(0.25+0.3+0.5)*100 percentage of neurons have `linear` activation function,
    0.3/(0.25+0.3+0.5)*100 percentage of neurons have `relu` activation function,
    0.5/(0.25+0.3+0.5)*100 percentage of neurons have `tanh_function` activation function:*/
    ActivationConfig layer2_configs[] = {
        {linear, 0.25},{relu, 0.3}, {tanh_function, 0.5}}; 

    /* INPUT:
    In layer3 (third layer or second hidden layer),
    100 percentage of neurons have `tanh_function` activation function: */
    ActivationConfig layer3_configs[] = {
        {tanh_function, 1}};

    /* INPUT:
    In layer4 (fourth layer or third hidden layer),
    5/(5+3)*100 percentage of neurons have `sigmoid` activation function,
    3/(5+3)*100 percentage of neurons have `tanh_function` activation function: */
    ActivationConfig layer4_configs[] = {
        {sigmoid, 5},{tanh_function, 3}};
    
    /* INPUT:
    The output layer (layer5),
    1/(1+1) proportion of neurons assigned to `linear` activation function,
    1/(1+1) proportion of neurons assigned to have `relu` activation function:
    */
    ActivationConfig layer5_configs[] = {
        {relu, 1},{linear, 1}};
